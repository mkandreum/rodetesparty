				 ...
				 try {
					...
					if (isPastEvent) {
						if (event.galleryImages && event.galleryImages.length > 0) {
							buttonHtml = `<button data-event-id="${event.id}" class="gallery-link-btn w-full bg-gray-700 text-white font-pixel text-2xl py-3 px-4 rounded-none border border-gray-600 hover:bg-gray-600 transition-colors duration-300">VER GALERÍA</button>`;
						 ...
						 if (capacity > 0 && sold >= capacity) {
							 ...
							 buttonHtml = `<button data-event-id="${event.id}" class="get-ticket-btn w-full bg-white text-black font-pixel text-2xl py-3 px-4 rounded-none border border-gray-400 hover:bg-gray-300 transition-colors duration-300">CONSEGUIR ENTRADA</button>`;
							 actionClass = 'get-ticket-btn cursor-pointer'; // <-- Acción clicable
						 }
					}

					const card = document.createElement('div');
					card.className = `relative bg-gray-900 rounded-none ${cardBorderColor} overflow-hidden flex flex-col transform transition-all hover:border-gray-300 hover:shadow-white/30 duration-300`;
					...
					card.innerHTML = `
						...
						<div class="p-6 flex flex-col flex-grow">
							...
							<h3 class="text-3xl font-pixel ${isPastEvent ? 'text-gray-500' : 'text-white text-glow-white'} mb-2 ${actionClass}" ${dataAttribute}>
			...
			eventsWithGalleries.forEach(event => {
				try {
					...
					card.innerHTML = `
						...
						<div class="p-6">
							<h3 class="text-3xl font-pixel text-white text-glow-white truncate">${event.name || 'Evento sin nombre'}</h3>
			...
			dragsToShow.sort((a, b) => (a.name || '').localeCompare(b.name || '')).forEach(drag => {
				try {
					...
					card.innerHTML = `
						...
						<div class="p-6 flex flex-col flex-grow">
							<h3 class="text-3xl font-pixel text-white text-glow-white mb-2 truncate">${drag.name || 'Drag sin nombre'}</h3>
							<p class="text-gray-400 mb-6 flex-grow">${drag.description || 'Sin descripción.'}</p>
							<div class="space-y-3">
								<button data-drag-id="${drag.id}" class="drag-gallery-btn w-full bg-white text-black font-pixel text-lg py-2 px-4 rounded-none border border-gray-400 hover:bg-gray-300 transition-colors duration-300 ${galleryCount === 0 ? 'hidden' : ''}">
